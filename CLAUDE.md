...
- **E2E notes**: wait for either the setup screen or the loaded app shell before deciding whether to create a new database; the Add Transaction dialog account field is an unnamed MUI select — target it structurally, not by accessible name; worker-recovery tests use `window.__budgetTrackerTestApi.disconnectWorker()` plus a full page reload.
